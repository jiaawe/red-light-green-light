...
class TrafficOptimizer:
    ...
    def __init__(
        ...
        self.traffic_config = self.load_json_file(traffic_config_path)[
            "movements_description"]  # Try using movements_descriptions only
    ...
    def optimize(
        ...
        try:
            ...
            required_keys = ["selected_configuration",
                             "duration_seconds", "justification"]
            if not all(key in result for key in required_keys):
                raise ValueError(
                    f"Response missing required keys: {required_keys}")
            ...
            if memory_to_use:
                updated_memory.extend(
                    memory_to_use[:min(5, len(memory_to_use))])
